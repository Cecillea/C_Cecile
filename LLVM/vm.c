...
static bool invoke(ObjString* name, int argCount) {
  ...
  if (!CECILE_IS_INSTANCE(receiver)) {
    runtimeError("Only instances have methods.");
    return false;
  }

  ObjInstance* instance = CECILE_AS_INSTANCE(receiver);
...
static void defineMethod(ObjString* name) {
  Value method = peek(0);
  ObjClass* klass = CECILE_AS_CLASS(peek(1));
  ...
  for (;;) {
    ...
    switch (instruction = READ_BYTE()) {
      ...
      case OP_INHERIT: {
        ...
        if (!CECILE_IS_CLASS(superclass)) {
          runtimeError("Superclass must be a class.");
          return INTERPRET_RUNTIME_ERROR;
        }

        ObjClass* subclass = CECILE_AS_CLASS(peek(0));
        tableAddAll(&CECILE_AS_CLASS(superclass)->methods, &subclass->methods);
        pop();
        break;
      }
      case OP_GET_SUPER: {
        ObjString* name = READ_STRING();
        ObjClass* superclass = CECILE_AS_CLASS(pop());
